-- ============================================
-- APPLY PERFORMANCE OPTIMIZATION VIEWS - CORRECTED
-- ============================================
...
-- Date: 2025-01-14
-- Status: CORRECTED - Based on actual database schema
...
LEFT JOIN water_tests wt ON wt.service_id = s.id
LEFT JOIN bookings b ON b.unit_id = u.id
GROUP BY p.id, p.name, p.property_type, p.address, p.is_active, p.created_at, p.company_id, p.has_individual_units, c.name;
...
LEFT JOIN water_tests wt ON wt.service_id = s.id
LEFT JOIN bookings b ON b.unit_id = u.id
GROUP BY u.id, u.name, u.unit_type, u.unit_number, u.is_active, u.created_at, u.property_id, u.customer_id, p.name, p.property_type, p.company_id, c.name, c.email, c.phone;
...
LEFT JOIN services s ON s.unit_id = u.id
LEFT JOIN bookings b ON b.unit_id = u.id
GROUP BY c.id, c.name, c.email, c.phone, c.address, c.customer_type, c.is_active, c.created_at, c.company_id, comp.name;
