...
import { updateTemplateForCompany, deleteTemplateForCompany } from '@/lib/data/templates'

export const dynamic = 'force-dynamic'

// ownership handled in DAL
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    try {
      const out = await updateTemplateForCompany(id, updates, companyId)
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!companyId) return NextResponse.json({ error: 'No company' }, { status: 400 })
    try {
      const out = await deleteTemplateForCompany(id, companyId)
      const res = NextResponse.json(out)
      res.headers.set('Server-Timing', `db;dur=${Date.now() - t0}`)
      return res
    } catch (err: any) {
