...
import { getUnitByIdForCompany, updateUnitForCompany, deleteUnitForCompany } from '@/lib/data/units'
...
export async function GET(
  ...
  try {
    ...
    try {
      const unit = await getUnitByIdForCompany(id, companyId)
...
export async function PATCH(
  ...
  try {
    ...
    if (!companyId) return NextResponse.json({ error: 'No company' }, { status: 400 })
    try {
      const unit = await updateUnitForCompany(id, updates, companyId)
      const res = NextResponse.json({ unit })
      res.headers.set('Server-Timing', `db;dur=${Date.now() - t0}`)
      return res
    } catch (err: any) {
...
export async function DELETE(
  ...
  try {
    ...
    if (!companyId) return NextResponse.json({ error: 'No company' }, { status: 400 })
    try {
      await deleteUnitForCompany(id, companyId)
