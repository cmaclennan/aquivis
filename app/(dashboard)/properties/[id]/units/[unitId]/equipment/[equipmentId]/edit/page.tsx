...
export default function EditUnitEquipmentPage({ params }: { params: Promise<{ id: string; unitId: string; equipmentId: string }> }) {
  const { id: propertyId, unitId, equipmentId } = use(params)
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        <Link href={`/properties/${propertyId}/units/${unitId}/equipment`} className="text-sm text-gray-600 hover:text-gray-900">← Back to Equipment</Link>
        <h1 className="text-2xl font-bold text-gray-900 mt-2">Edit Equipment</h1>
      </div>
      ...
      <form onSubmit={save} className="bg-white rounded-lg shadow p-6 max-w-2xl">
        ...
        <div className="flex items-center justify-end gap-2">
          <Link href={`/properties/${propertyId}/units/${unitId}/equipment`} className="px-4 py-2 border rounded">Cancel</Link>
