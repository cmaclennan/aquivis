...
export default function NewPlantRoomPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true)
      setError(null)
      // Convert day names to integers for DB compatibility (0=Sunday .. 6=Saturday)
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900 mt-2">New Plant Room</h1>
      </div>
      ...
      <form onSubmit={save} className="bg-white rounded-lg shadow p-6 max-w-2xl">
        ...
        <div className="flex items-center justify-end gap-2">
          ...
          <button type="submit" disabled={loading} className="px-4 py-2 bg-primary text-white rounded disabled:opacity-50">{loading ? 'Saving…' : 'Save'}</button>
        </div>
      </form>
    </div>
  )
